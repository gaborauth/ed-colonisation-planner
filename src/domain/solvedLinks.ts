...
// display. This module merges `result.placements` with the same present-facilities-to-placements
// conversion `presentLinks.ts` already uses for the present-only case, excluding whatever the
// solver actually demolished (`result.demolished`) — those slots are no longer "present" once a
// fresh solve says so.
 ...
 * newly solved for) — any caller of a solved plan's link topology should use this, not
 * `computeSystemLinks` with `result.placements` directly. */
